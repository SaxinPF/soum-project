  ...
  if($admin=='Admin')
  {
     ...
     $devicemodal='';
						...
						else
						{
							...
							$sql=$db->prepare("update soum_product_master set post_date='$post_date',titile='$titile',rate='$mrp',brand='$drpBrand',modal='$drpModel',year_purchase='$txt_year',imei_no='$imi_no',rom='$mrom',ram2='$ram',color_id='$color_id',Constituent='$txt_description',appliable_rate='$offer_price',offer_price='$offer_price',source='$source',source_id='$source_id',pincode='$pincode',lat='$lat',lng='$lng',opening_stock='$qty',current_stock='$qty',yearbyadmin='$admin_year',category='$devicecat',condi='$condi1',condi2='$condi2',warranty='$warranty',active='$approve',priority='$setp' where prod_id=$prodid");
						    $res=$sql->execute();
						    $enq_id=$prodid;
						/***************Image1************************************/
									if($imageData1!='' && $imageData1!='no_img.png')
