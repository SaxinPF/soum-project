...
$layout_title = 'New/Used Tablets - SOUM';
    $pht=$_REQUEST['phone'];
    ...
    $tablet_condition_value_condition = '';
    $query_string = parse_url($current_url, PHP_URL_QUERY);
    parse_str($query_string, $query_params);
    if (isset($query_params['tablet_condition'])) {
        $tablet_condition_value = $query_params['tablet_condition'];
      // echo $phone_condition_value;
        $tablet_condition_value_condition = 'and soum_product_master.prod_cat_id = '.$tablet_condition_value;
    }
        ...
        <div class="filter">
          <div class="container form-inline" >
            <div class="col-sm-12">
                      ...
                      <select name="tablet_condition" id="tablet_condition"  style="width: 150px; text-align: center;">
                        <option>Select New/Used</option>
                        <option value="1" <?php if ($tablet_condition_value == '1') echo 'selected'; ?>>New</option>
                        <option value="2" <?php if ($tablet_condition_value == '2') echo 'selected'; ?>>Used</option>
                      </select>
...
<?php
  ...
  if($_REQUEST['tablet_condition']){
    $co = $_REQUEST['tablet_condition'];
          $searchArray[]=$co;
   }
...
and soum_product_master.category_type = 'tablet'
and soum_product_master.active=1 and soum_product_master.trash!='delete' and soum_product_master.dispatched_date >$time and soum_product_master.dealer_del_date >$time  and soum_product_master.prod_id!=0 ". $tablet_condition_value_condition . $cond1." ".$cond2." ".$cond;  
...
<div class="text-center"> <?php
       		...
       		<div class="styled-pagination margin-bott-40">
                <ul>
                      <li><a  class="<?php if($active==1){ $class='active';} else { $class='prev1';} ?>"  href='tablets.php?page=1&<?=$params;?>'><span class="fa fa-angle-left"></span>&ensp;First</a></li>
 <?php
...
and soum_product_master.active=1 and soum_product_master.trash!='delete' and soum_product_master.dispatched_date >$time and soum_product_master.dealer_del_date >$time  and soum_product_master.prod_id!=0 ". $tablet_condition_value_condition . $cond1." ".$cond2." ".$cond;  
								...
								$total_pages = ceil($total_records / $num_rec_per_page);
                                   for ($i=1; $i<=$total_pages; $i++)
									...
									echo "<li><a class=".$class." href='tablets.php?page=".$i."&".$params1."'>".$i."</a></li>";
                                   }
                                   ?>
                                <li><a class="<?php if($active==$total_pages){ $class='active';} else { $class='next1';} ?>" href='tablets.php?page=<?=$total_pages."&".$params1;?>'>Last&ensp;<span class="fa fa-angle-right"></span></a></li>
                            </ul>
            </div> </div>
