<?php include('config.php');
  ...
  $proid = mysqli_real_escape_string($db,$_GET['proid']);
  
     $sql_p ="select *,soum_prod_subcat.prod_subcat_desc as brand_name,soum_prod_subsubcat.prod_subcat_desc as model_name,soum_product_master.warranty warr from soum_product_master,soum_prod_subcat,soum_prod_subsubcat where
...
//echo $poster_type."a";
	...
	$layout_title = 'SOUM | Services Online Exchange Mobile';
?>
<style>
...
<html lang="en">
 ...
 <?php 
	...
	$brand_proname = $row_pro['brand_name'];
	$model_proname = $row_pro['model_name'];
	?><h3>Exchange with <?php echo $brand_proname .' '.$model_proname; ?></h3>
 <?php
 if(empty($poster_id) || $poster_id==''){ ?>
 Looks like you are not logged into the site. Press <a href="#" id="lognew" data-toggle="modal" data-target="#exampleModal">Log In</a> to continue.
  <script> 
...
<div class="row">
<div class="col-sm-8">
				 <form name="myForm" onsubmit="return valid4()" method="post" action="exchange_save.php">
					...
					<input name="order_prod_id"   type="hidden" value="<?=$proid;?>" class="form-control" />
					<input name="order_prod_name" type="hidden" value="<?php echo $brand_proname .' '.$model_proname; ?>" class="form-control" />
