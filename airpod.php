...
$layout_title = 'SOUM | Airpods';
    ...
    $airpod_condition_value_condition = '';
    $query_string = parse_url($current_url, PHP_URL_QUERY);
    parse_str($query_string, $query_params);
    if (isset($query_params['airpod_condition'])) {
        $airpod_condition_value = $query_params['airpod_condition'];
      // echo $phone_condition_value;
        $airpod_condition_value_condition = 'and soum_product_master.prod_cat_id = '.$airpod_condition_value;
    }
...
<html lang="en">
   <head>
     <title>New/Used Airpods & Earbuds - SOUM</title>
    <!-- Required meta tags -->
      ...
      <div class="mainhead ">
        <div class="filter">
          <div class="container form-inline" >
            <div class="col-sm-12">
                      <select name="airpod_condition" id="airpod_condition"  style="width: 150px; text-align: center;">
                        <option>Select New/Used</option>
                        <option value="1" <?php if ($airpod_condition_value == '1') echo 'selected'; ?>>New</option>
                        <option value="2" <?php if ($airpod_condition_value == '2') echo 'selected'; ?>>Used</option>
                      </select>
...
<?php
  /*BOF Security Patch IS-002*/
    ...
    if(isset($_REQUEST['brand_product_name']) && $_REQUEST['brand_product_name']!=''){
      ...
      if($_REQUEST['airpod_condition']){
        $co = $_REQUEST['airpod_condition'];
        $searchArray[]=$co;
      }  
...
and soum_product_master.category_type = 'airpod'
and soum_product_master.active=1 and soum_product_master.trash!='delete' and soum_product_master.dispatched_date >$time and soum_product_master.dealer_del_date >$time  and soum_product_master.prod_id!=0 ". $airpod_condition_value_condition . $cond1." ".$cond2." ".$cond;  
...
<div class="text-center"> <?php
       		...
       		<div class="styled-pagination margin-bott-40">
                <ul>
                      <li><a  class="<?php if($active==1){ $class='active';} else { $class='prev1';} ?>"  href='airpod.php?page=1&<?=$params;?>'><span class="fa fa-angle-left"></span>&ensp;First</a></li>
 <?php
...
and soum_product_master.active=1 and soum_product_master.trash!='delete' and soum_product_master.dispatched_date >$time and soum_product_master.dealer_del_date >$time  and soum_product_master.prod_id!=0 ".$airpod_condition_value_condition . $cond1." ".$cond2." ".$cond;  
								...
								$total_pages = ceil($total_records / $num_rec_per_page);
                                   for ($i=1; $i<=$total_pages; $i++)
									...
									echo "<li><a class=".$class." href='airpod.php?page=".$i."&".$params1."'>".$i."</a></li>";
                                   }
                                   ?>
                                <li><a class="<?php if($active==$total_pages){ $class='active';} else { $class='next1';} ?>" href='airpod.php?page=<?=$total_pages."&".$params1;?>'>Last&ensp;<span class="fa fa-angle-right"></span></a></li>
                            </ul>
            </div> </div>
