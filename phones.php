...
<html lang="en">
   <head>
   <title>Buy Second hand Mobile Phones in Jaipur - SOUM</title>


<meta name="description" content="We provide wide range of high-quality second hand mobile phones in Jaipur. Best Price! Top Brands! Call +91-9828075008 for queries."/>
<link rel="canonical" href="https://www.soum.co.in/second-hand-phone-jaipur" />
    <!-- Required meta tags -->
...
<script>
 ...
 <h1>Second hand mobile phones</h1>
 <div class="clearfix"></div>
 <div class="row">
...
if($disc_perc == 0){
    ...
    $newprice = "SELECT MIN(price) AS min_price ,color_id FROM product_variation WHERE prod_id =" . $row['prod_id']. " AND price > 0";
    ...
    //echo $img_prod;
    $clid = $rowpr['color_id'];
     if(strpos($img_prod, 'modelImage') === 0 || $img_prod == '0' || empty($img_prod) || strpos($img_prod, 'noimage.png') !== false){
...
<div class="col-lg-3">
 <div class="prodcat" onclick="window.open('<?php echo SITEPATH. slugify($row['brand'].'-'.$row['model']).'-'.$row['prod_id'];?>','_blank')" style="cursor:pointer;">
		...
		<div class="prolstdtl"><div class="row">
			<!-- 	<div class="col-sm-12 catpric"><?php if($row['availability']=='Unavailable'){ ?>Starting from <?php } ?> <img src="img/rupees.png" alt=""/><?=$disc_perc;?>/- </div>
 -->
	...
	<div class="clearfix"></div>
	 <div class="montold"><?php if($row['availability']=='Unavailable'){ ?><span class="restock">Restock in 2 days</span><?php } else { ?><span class="instock"><?php if($row['current_stock']<=0){ echo  $row['yearbyadmin'];}else{ echo $row['yearbyadmin'];}?></span> <?php } if($img_logo=="Admin"){?><figure><img src="img/symb.png" alt=""/></figure><?php } ?></div>
	 <div class="catfeat">
	 <ul>
       <?php
...
<div class="text-center"> <?php
       		...
       		<div class="styled-pagination margin-bott-40">
                <ul>
                      <li><a  class="<?php if($active==1){ $class='active';} else { $class='prev1';} ?>"  href='second-hand-phone-jaipur?page=1&<?=$params;?>'><span class="fa fa-angle-left"></span>&ensp;First</a></li>
 <?php
...
and soum_product_master.active=1 and soum_product_master.trash!='delete' and soum_product_master.dispatched_date >$time and soum_product_master.dealer_del_date >$time ".$cond1." ".$cond2." ".$cond;
								...
								$total_pages = ceil($total_records / $num_rec_per_page);
                                   for ($i=1; $i<=$total_pages; $i++)
									...
									echo "<li><a class=".$class." href='second-hand-phone-jaipur?page=".$i."&".$params1."'>".$i."</a></li>";
                                   }
                                   ?>
                                <li><a class="<?php if($active==$total_pages){ $class='active';} else { $class='next1';} ?>" href='second-hand-phone-jaipur?page=<?=$total_pages."&".$params1;?>'>Last&ensp;<span class="fa fa-angle-right"></span></a></li>
                            </ul>
            </div> </div>
...
<div style="background: #fafafa;padding: 40px;" class="mainhead"><div class="headslog container"><h1>Looking for something else? No problem. <span>Drop a enquiry by clicking the button below and our expert would assist you in the best possible way.</span></h1>
  <a class="seehowmuch" href="/tell_us.php" role="button"> Enquire now</a></div></div>
