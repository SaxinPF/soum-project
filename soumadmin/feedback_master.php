		...
		<div class="content-container" id="content">
			...
			<div class="page page-dashboard">
				...
				<div class="page-wrap">
					...
					<div class="row">
						...
						<div class="col-md-12">
							<div class="dash-head clearfix mb20">
								<div class="left">
									<h4 class="mb5 text-light">Feedback/ Contact Us Master</h4>
								</div>
							</div>
						</div>
					</div> <!-- #end row -->
					...
					{
					$feedbacid=$_REQUEST['feedback'];
					 $sql="select * from soum_feedback where feedback_id=$feedbacid";
					  $res=$db->query($sql);
					 while($row=$res->fetch_assoc())
					 {
					  $pid=$row['prod_id'];
					  $date=$row['dttm'];
					  $enq_typ=$row['enquiry_type'];
					  $name=$row['fname'];
					  $email=$row['email'];
					  $sub=$row['subject'];
					  $msg=$row['msg'];
					...
					<div class="row" id="form_offer">
						<!-- dashboard header -->
						<form method="post" action="feedback_master_act.php" enctype="multipart/form-data">
						<input name="feedback" type="hidden" value="<?=$feedbacid?>"/>
						...
						<div class="col-md-6" style="margin:auto;float:none;">
							<div class="dash-head clearfix mb20">
									...
									<div class="col-md-6">
										<label class="control-label small">Enquiry</label>
										<input class="form-control" type="text" id="enq_typ" name="enq_typ" value="<?=$enq_typ?>">
									</div>
									<div class="col-md-6">
									...
									<div class="col-md-6">
										...
										<input class="form-control" type="text" id="sub" name="sub" value="<?=$sub?>">
									</div>
									<div class="col-md-12">
										<label class="control-label small">Message</label>
										<textarea rows="3" class="form-control resize-v" placeholder="Message here" id="msg" name="msg"><?=$msg?></textarea>	
									</div>
									...
									<a href="feedback_master_act.php?feedback=<?=$feedbacid?>&act=del" class="btn btn-primary mr5 waves-effect">Delete</a>
					...
					<div class="row">
						...
						<div class="dash-head clearfix mb20" style="min-height:450px;">
						 ...
						 if(isset($_REQUEST['Submit1']))
						 {
                           ...
                           $cond=" and  concat(dttm,enquiry_type,fname,email,subject,msg) like '%$search%'"; 
						 }
						?>
						...
						<table class="table table-bordered invoice-table mb30" id="list-1">
										<thead>
											<tr style="background: #38b4ee;color: #fff">
												...
												<th style="padding: 5px;">Enquiry</th>
												<th style="padding: 5px;">ame</th>
												<th style="padding: 5px;">Email</th>
												<th style="padding: 5px;">Subject</th>
												<th style="padding: 5px;">Message</th>
												<th style="padding: 5px;width: 150px;">Action <a href="feedback_master.php?act=add"><button class="btn btn-primary mr5 waves-effect" type="submit" style="margin:0px;padding:0px 8px;float:right;">[+]</button></a></th>
											</tr>
										</thead>
									<?php
									...
									$sql="select * from soum_feedback where 1=1 ".$cond." LIMIT $start_from, $num_rec_per_page";
									$res=$db->query($sql);
									$i=1;
									while($row=$res->fetch_assoc())
									{
									?>
										<tr>
											...
											<td style="padding: 5px;"><?=$row['dttm'];?></td>
											<td style="padding: 5px;"><?=$row['d_t'];?></td>
											<td style="padding: 5px;"><?=$row['enquiry_type'];?></td>
											<td style="padding: 5px;"><?=$row['fname'];?></td>
											<td style="padding: 5px;"><?=$row['email'];?></td>
											<td style="padding: 5px;"><?=$row['subject'];?></td>
											<td style="padding: 5px;"><?=$row['msg'];?></td>
											<td style="padding: 5px;">
											<a href="feedback_master.php?feedback=<?=$row['feedback_id']?>&act=edit" class="link btn-primary">Edit</a>
...
$sql = "select * from soum_feedback wehre 1=1 ".$cond; 
$rs_result =$db->query($sql); //run the query
$total_records = mysqli_num_rows($rs_result);  //count number of records
//echo $total_records;
$total_pages = ceil($total_records / $num_rec_per_page); 
echo "<a style='background-color: rgb(255, 45, 0);color:#fff;padding: 1px 4px;' href='feedback_master.php?page=1&$params'>".'First'."</a> "; // Goto 1st page  
for ($i=1; $i<=$total_pages; $i++) { 
            echo "<a style='background-color: rgb(255, 45, 0);color:#fff;padding: 1px 4px;' href='feedback_master.php?page=".$i."&".$params."'>".$i."</a> "; 
}; 
echo "<a style='background-color: rgb(255, 45, 0);color:#fff;padding: 1px 4px;' href='feedback_master.php?page=$total_pages&$params'>".'Last'."</a> "; // Goto last page
?>
</div>
