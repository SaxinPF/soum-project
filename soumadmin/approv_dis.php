...
.expirered{color:red;font-weight: bold;}
.expiregreen{color:green;font-weight: bold;}
</style>
								...
								<div style="width:100%;float:left">
									<div class="ui-radio ui-radio-pink">
										...
										<label class="ui-radio-inline" style="padding: 10px 0px;font-weight: bold;font-size: 14px;width: auto;" onclick="gopage('dealer');">
											<input name="radioEg" type="radio" <?php echo "checked";?>>
											<span>Dealer</span>
										</label>
								...
								<div class="tab-content table-responsive">
									...
									<thead>
										<tr style="background:#4a9de4;color: #fff">
											...
											<th style="padding:5px;">Expiry Date</th>
...
if(poster_type='Dealer',
 ...
 from (select   temp.*,soum_prod_subsubcat.prod_subcat_desc model_name from(
										...
										,soum_product_master.poster_id,soum_product_master.poster_type,soum_product_master.post_date,soum_product_master.code,soum_product_master.offer_price,soum_product_master.dealer_del_date from soum_product_master where active=0 $cond) prod
									...
									?>
										<tr>
											...
											<td><?=$row['offer_price'];?></td>
												    <?php  $timeto = strtotime(date('d-m-Y')); 
														$expirecls  = "expirered";
														if($row['dealer_del_date'] >= $timeto){
														   $expirecls  = "expiregreen";
														}        
						                               ?>
					                         <td class="<?php echo $expirecls; ?>"><?php echo date('d-m-Y',$row['dealer_del_date']); ?></td>
											<td><a class="link btn-primary" href="phone_detail.php?prod_id=<?=$row['prod_id'];?>&poster_id=<?=$row['poster_id'];?>&type=<?=$type;?>">Details</a>
                                             <a class="link btn-danger" onclick="return confirm('Are you sure you want to delete this product ?');"href="product_trash.php?prod_id=<?=$row['prod_id'];?>&type=Dealer">Delete</a>
...
<table class="table table-bordered invoice-table mb30" id="list-1">
									<thead>
										<tr style="background: #4a9de4;color: #fff">
											...
											<th style="padding:5px;">Brand Model</th>
											<th style="padding:5px;">Expected Price</th>
                                            <th style="padding:5px;">Transferred</th>
                                            <th style="padding:5px;">Expiry Date</th>
											<th style="padding:5px;">See Details</th>
										</tr>
									</thead>
...
if(poster_type='Dealer',
 ...
 from (select   temp.*,soum_prod_subsubcat.prod_subcat_desc model_name from(
										select * from (
										select soum_product_master.prod_id,if(soum_product_master.prod_cat_id=1,'New','Used') prod_type,soum_product_master.prod_cat_id,soum_product_master.rate,soum_product_master.brand,soum_product_master.modal
										,soum_product_master.poster_id,soum_product_master.poster_type,soum_product_master.post_date,soum_product_master.code,soum_product_master.offer_price,soum_product_master.transfer,soum_product_master.dealer_del_date from soum_product_master where active=1 $cond) prod
										left outer join soum_prod_subcat
										on prod.brand=soum_prod_subcat.prod_subcat_id)temp
										left outer join soum_prod_subsubcat
										on temp.modal=soum_prod_subsubcat.prod_subsubcat_id
