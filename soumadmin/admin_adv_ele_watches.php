				...
				if(empty($error)){
						...
						if($_POST['act' ] == "add"){
							...
							$table=(!isset($poster_type)?'soum_product_master_temp':'soum_product_master');
								...
								$moda ='Null';
								 ...
								 $s3='watches';
									...
									values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
									$sql->bind_param("ssssssssssssssssssssssssss",$post_date,$titile,$condition,$drpBrand,$moda,$drpModel,$imi_no,$img1,$img2,$img3,$description,$poster_type,$poster_id,$approve,$offer_price,$offer_price,$warranty,$color_name,$seller_name,$seller_email,$seller_phone,$seller_address,$seller_city,$s1,$s2,$s3);
                            ...
                            // echo '<script>window.location.href="admin_adv_cable.php?req_id='.$enq_id.'&act=add&step=2&admin_adv_s=Please add your images."</script>';
                             echo '<script>window.location.href="admin_adv_watches.php?admin_adv_s=Ad has been updated successfully"</script>';
				      }
					  if($_POST['act' ] == "edit"){
							$qty =1;
	                        $sql=$db->prepare("update soum_product_master set titile='$titile',prod_cat_id = '$condition' ,brand='$drpBrand',modal_name='$drpModel',images='$img1',images1='$img2',images2='$img3',imei_no='$imi_no',appliable_rate='$offer_price',active=$approve,offer_price='$offer_price',warranty='$warranty',seller_name='$seller_name',seller_email='$seller_email',seller_phone='$seller_phone',color_name='$color_name',seller_address='$seller_address',seller_city='$seller_city',opening_stock='$qty',current_stock='$qty',description='$description' where prod_id=$prodid");
						    $res=$sql->execute();
						    $enq_id=$prodid;
							...
							<form id="uploadimage"  method="post" enctype="multipart/form-data">
									...
									<div class="col-md-6">
							                ...
							                <select class="form-control" name="condition" id="condition" >
												...
												<option value="1" <?php  if($condition == '1') { ?> selected="selected" <?php } ?>>New</option>
												<option value="2" <?php  if($condition == '2') { ?> selected="selected" <?php } ?>>Used</option>
											</select>
									</div>
									<div class="col-md-6">
