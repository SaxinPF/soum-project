...
<script>
function edit(cat_id,catename,home,logo,active1,act,categories)
{
	$("#debugConsole1").val(logo);
	$("#debugConsole2").val(home);


	$("#file1").val(logo);
	if(logo=="")
	{
		$('#previewing1').attr('src', "../upload/c_logo/no_img.png");
	}
	$('#previewing1').attr('src',"../upload/c_logo/"+logo);


	$("#file3").val(home);
	if(home=="")
	{
		$('#previewing2').attr('src', "../upload/c_logo/no_img.png");
	}
	$('#previewing2').attr('src',"../upload/c_logo/"+home);


	$("#cat_id").val(cat_id);
    $("#act").val(act);
	$("#catname").val(catename);
	//alert(categories);
	var a1 = new Array();
...
if (name=='')
{
	alert('Brand name must be filled!');
	return false;
}

if (img1=='' || img1=='0')
{
	alert('Home Logo must  be selected!');
	return false;
}

if (img2=='' || img2=='0')
{
	alert('Product Logo must  be selected!');
	return false;
}
		...
		if(act_conf=='Delete')
		{
	 	...
	 	var catname= document.getElementById("catname").value;
	  	...
	  	if(act_conf=='Edit' || act_conf=='Add')
	  	{
		  	if(catname=='')
		  	{
		  		alert("Company/ Brand Name must be Filled");
		  		return false;
		  	}
		}
		$.ajax({
		url: "subcat_master_act.php", // Url to which the request is send
				...
				<div class="page-wrap">
					<div class="row">
						...
						<div class="col-md-12" style="margin-top:30px;">
							<div class="dash-head clearfix mb20">
								<div class="left">
									<h4 class="mb5 text-light"><a name="top">Company/ Brand Master</a></h4>
								</div>
							</div>
						</div>
					</div>
						...
						<div class="col-md-6" id="enq" style="margin:auto;float:none;display:none">
							<div class="dash-head clearfix mb20">
									...
									<label class="control-label">Select Categories</label>
									 ...
									 <?php $category_array = get_category_types(); 
									...
									<div class="col-md-12" style="margin-top:10px;">
										<label class="control-label">Company/ Brand Home Logo <span style="font-size:12px;">(image dimensions 110px X 82px)</span></label>
									</div>
									<div class="col-sm-4" style="margin-top:15px;">
										<div class="filedivcls">
											<img id="previewing1" src="../images/no_img.png"  style="height:80px;width:100%;position:absolute;top:0;"/>
											<span class="select-wrapper"><input name="file"  id="file" onchange="abc(this,1);" type="file" style="width:100%;margin-bottom:13px;text-align:left;padding:20px 5px;border: 1px dashed #c5d7b5;height:auto;background-color:#dcf9ff;"></span>
										</div>
										<input type="hidden" id="file1">
									</div>

									<div class="col-md-12" style="margin-top:10px;">
										<label class="control-label">Company/ Brand Product Logo <span style="font-size:12px;">(image dimensions 110px X 82px)</span></label>
									</div>
									<div class="col-sm-4" style="margin-top:15px;">
										<div class="filedivcls">
											<img id="previewing2" src="../images/no_img.png"  style="height:80px;width:100%;position:absolute;top:0;"/>
											<span class="select-wrapper"><input name="file2"  id="file2" onchange="abc(this,2);" type="file" style="width:100%;margin-bottom:13px;text-align:left;padding:20px 5px;border: 1px dashed #c5d7b5;height:auto;background-color:#dcf9ff;"></span>
										</div>
									...
									<button class="btn btn-primary mr5 waves-effect" type="submit" style="margin-top:20px;" id="btn-add" />
								    <button class="btn btn-primary mr5 waves-effect" style="display:none;margin-top:20px;" id="delete" type="button" onclick="del()" value="Delete">Delete</button>
									</div>
									<p>&nbsp;</p>
					...
					<?php
						 if(isset($_REQUEST['Submit1']))
						 {
                           ...
                           $cond=" and  concat(prod_subcat_desc) like '%$search%'";
						 }
						?>
					<div class="row">
						<div class="col-md-12">
						<!-- <div class="box-body table-responsive no-padding"> -->
							 ...
							 <table style="width:auto;float:right;">
								...
								<table class="table table-bordered invoice-table mb30" id="list-1" style="margin-top:15px; width: 100%;">
									<thead>
										<tr style="background: #38b4ee;color: #fff">
										    <th style="padding: 5px;">SNo.</th>
											<th style="padding: 5px;">Company/ Brand Name</th>
											<th style="padding: 5px;">Home Image</th>
											<th style="padding: 5px;">Product Image</th>
											<th style="padding: 5px;">Active</th>
											<th style="padding: 5px;width:160px;">Action <button class="btn btn-primary mr5 waves-effect" id="Add" type="submit" style="margin:0px;padding:0px 8px;float:right;" onclick="edit('','','','','','Add')">[+]</button></th>
										</tr>
									</thead>
									<tbody>
										<?php
										$sql="select * from soum_prod_subcat where prod_subcat_id!=0 ".$cond;
										$res=$db->query($sql);
										...
										while($row=$res->fetch_assoc())
										{
										         $prod_subcat_id = $row['prod_subcat_id'];
												  $sql="SELECT categroy_type
                                                       FROM categroy_brands WHERE brand_id=$prod_subcat_id";
		                                               $res_cat = $db->query($sql);
													   $category_type ='';
										                while($vlaue=$res_cat->fetch_assoc()){
														  $category_type .= $vlaue['categroy_type'].',';
														}
										...
										?>
											<tr>
												...
												<td style="padding: 5px;"><img src="../upload/c_logo/<?=$row['logo'];?>" style="width:50px;height:auto"></td>
												<td style="padding: 5px;"><img src="../upload/c_logo/<?=$row['home_logo'];?>" style="width:50px;height:auto"></td>
												<td style="padding: 5px;"><?=$row['priority1'];?></td>
												<td style="padding: 5px;">
													<button class="link btn-primary Edit" type="submit" style="margin:0px;padding: 2px 12px;" onclick="edit('<?=$row['prod_subcat_id'];?>','<?=$row['prod_subcat_desc'];?>','<?=$row['home_logo'];?>','<?=$row['logo'];?>','<?=$row['priority1'];?>','Edit','<?php echo $category_type ?>')">Edit</button>
												</td>
											</tr>
										<?php
										}
