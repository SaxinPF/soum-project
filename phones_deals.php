...
<?php
  ...
  $cond1='';
   ...
   if(isset($_REQUEST['category_type']) && $_REQUEST['category_type']!=''){
       $category_type=$_REQUEST['category_type'];
       $bindType .= 's' ;
       $cond1 .= '  and soum_product_master.category_type=?' ;
       $searchArray[]=$category_type;
     }
...
and soum_product_master.active=1 and soum_product_master.trash!='delete' and soum_product_master.dispatched_date >$time and soum_product_master.dealer_del_date >$time and soum_product_master.hot_deal=1 and  soum_product_master.prod_id!=0".$cond1." ".$cond2." ".$cond;
 ...
 <div class="prodcat" onclick="window.location.href='<?php echo SITEPATH. slugify($row['brand'].'-'.$row['model']).'-'.$row['prod_id'];?>'" style="cursor:pointer;">
	...
	<div class="clearfix"></div>
	 ...
	 <ul>
       ...
       <?php break;
       case 'cable':?>
          <li><strong>Length : </strong>&nbsp;<?php echo $row['length'];?></li>
      <?php break;
     } ?>
	 </ul>
	 </div>
