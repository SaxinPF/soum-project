...
if($json['error'] ==0){
			...
			$used='used';
				...
				$qry=$db->prepare("insert into soum_order_master (order_date,order_type,cust_id,cust_type,fname, mail, mobile, shipping_address,active,description,exchange)values(?,?,?,?,?,?,?,?,?,?,?)");
				$qry->bind_param("sssssssssss",$dt,$used,$id,$user_type,$fname,$mail, $mobile, $address,$a,$desc,$a);
				$res=$qry->execute();
			if($res)
			{
					...
					$qty=1;
						...
						$sql->bind_param("ssss",$ord_id,$prod_id,$qty,$price);
  						...
  						if($result)
  						{
							...
							//$msg = "This is your one time OTP .".$OTP;
