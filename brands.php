...
$layout_title = 'SOUM | Services Online Used Mobile';
    ...
    $sort=$_REQUEST['sort_act'];
        $brandName = humanize($_REQUEST['product_name']);

          $sql = "select * from soum_prod_subcat where prod_subcat_desc ='".$brandName."'";
    	  $resb=$db->query($sql);
		  while($brandrow = $resb->fetch_assoc())
		  {
		   $_REQUEST['drpBrand'] =  $brandrow['prod_subcat_id'];
		  }
...
$phone_condition_value_condition ='';
$query_string = parse_url($current_url, PHP_URL_QUERY);
parse_str($query_string, $query_params);
if (isset($query_params['phone_condition'])) {
    $phone_condition_value = $query_params['phone_condition'];
   // echo $phone_condition_value;
    $phone_condition_value_condition = 'and soum_product_master.prod_cat_id = '.$phone_condition_value;
}
?>
...
<script>
 ...
 <div class="clearfix"></div>
   <?php //include('elements/leftfilter.php');?>
 <div class="clearfix"></div>
    ...
    <div class="filter">
      <div class="container form-inline" >
        <div class="col-sm-12">
           ...
           <select name="phone_condition" id="phone_condition"  style="width: 150px; text-align: center;">
                    <option value="2" <?php if ($phone_condition_value == '2') echo 'selected'; ?>>Select New/Used</option>
                    <option value="1" <?php if ($phone_condition_value == '1') echo 'selected'; ?>>New</option>
                    <option value="2" <?php if ($phone_condition_value == '2') echo 'selected'; ?>>Used</option>
                  </select>
 ...
 <h2><?php echo $brandName;?> GADGETS </h2>
 <div class="clearfix"></div>
 <div class="row">
...
<?php
  ...
  $cond1='';
    ...
    if($_REQUEST['drpBrand']!='' && $_REQUEST['drpBrand']!='0')
     ...
     if($_REQUEST['phone_condition']){
      $co = $_REQUEST['phone_condition'];
            $searchArray[]=$co;
     }
  ...
  $sql = "select SQL_CALC_FOUND_ROWS
        *,soum_prod_subcat.prod_subcat_desc as brand,soum_prod_subsubcat.prod_subcat_desc as model,if(priority=0,if(soum_product_master.poster_type='Admin',1,if(soum_product_master.poster_type='Dealer',2,3)),0) p_type, if (soum_product_master.prod_cat_id=1, soum_prod_subsubcat.p_img1, soum_product_master.images) imagesx,
        ...
        and soum_product_master.active=1 and soum_product_master.trash!='delete' and soum_product_master.dealer_del_date >$time and soum_product_master.prod_id!=0 ".$phone_condition_value_condition . $cond1." ".$cond2." ".$cond;
        ...
        *,soum_prod_subcat.prod_subcat_desc as brand,soum_prod_subsubcat.prod_subcat_desc as model,if(priority=0,if(soum_product_master.poster_type='Admin',1,if(soum_product_master.poster_type='Dealer',2,3)),0) p_type, if (soum_product_master.prod_cat_id=1, soum_prod_subsubcat.p_img1, soum_product_master.images) imagesx,
        ...
        where soum_product_master.brand=  soum_prod_subcat.prod_subcat_id
        and soum_product_master.modal= soum_prod_subsubcat.prod_subsubcat_id
        and soum_product_master.active=1 and soum_product_master.trash!='delete' and soum_product_master.dispatched_date >$time and soum_product_master.dealer_del_date >$time and soum_product_master.prod_id!=0 ".$phone_condition_value_condition . $cond1." ".$cond2." ".$cond;
          ...
          /*EOF Security Patch IS-002*/
            ...
            *,soum_prod_subcat.prod_subcat_desc as brand,soum_prod_subsubcat.prod_subcat_desc as model,if(priority=0,if(soum_product_master.poster_type='Admin',1,if(soum_product_master.poster_type='Dealer',2,3)),0) p_type, if (soum_product_master.prod_cat_id=1, soum_prod_subsubcat.p_img1, soum_product_master.images) imagesx,
            ...
            //echo $img_prod;
                ...
                $clid = $rowpr['color_id'];
                 if(strpos($img_prod, 'modelImage') === 0 || $img_prod == '0' || empty($img_prod) || strpos($img_prod, 'noimage.png')){
...
<div class="col-lg-3">
 <div class="prodcat" onclick="window.location.href='<?php echo SITEPATH. slugify($row['brand'].'-'.$row['model']).'-'.$row['prod_id'];?>'" style="cursor:pointer;">
		...
		<div class="prothumd testbrand"><img src="upload/<?=$img_prod;?>" alt=""/></div>
		<div class="clearfix"></div>
	...
	<div class="clearfix"></div>
  <?php if($phone_condition_value == 2 || $phone_condition_value !== ''){?>
	    <?php
       switch($row['product_category']){
        case 'phone':?>
	       <div class="montold"><span><?php if($row['current_stock']<=0){ echo  $row['yearbyadmin'];}else{ echo $row['yearbyadmin'];}?></span> <?php if($img_logo=="Admin"){?><figure><img src="img/symb.png" alt=""/></figure><?php } ?></div>
        	<div class="catfeat">
               <ul>
                 <?php
                 switch($row['product_category']){
...
<h2 style="padding-bottom:50px;">Currently, we are out of stock for your selection. <span style="color:#5390fa;">We might refill your selection soon.</span> For more updates please call us at +91-9828075008</h2>
</div>
<?php } ?>
 </div>
 <?php  /*if($res->num_rows>0){ ?>
<div class="text-center"> <?php
       		...
       		<div class="styled-pagination margin-bott-40">
                <ul>
                      <li><a  class="<?php if($active==1){ $class='active';} else { $class='prev1';} ?>"  href='<?php echo SITEPATH.slugify($brandName).'-gadgets';?>?page=1'><span class="fa fa-angle-left"></span>&ensp;First</a></li>
 <?php
...
and soum_product_master.active=1 and soum_product_master.trash!='delete' and soum_product_master.dispatched_date >$time and soum_product_master.dealer_del_date >$time  and soum_product_master.prod_id!=0".$cond1." ".$cond2." ".$cond;
								...
								$total_pages = ceil($total_records / $num_rec_per_page);
                                   for ($i=1; $i<=$total_pages; $i++)
									...
									echo "<li><a class=".$class." href='".SITEPATH.slugify($brandName)."-gadgets?page=".$i."'>".$i."</a></li>";
                                   }
                                   ?>
                                <li><a class="<?php if($active==$total_pages){ $class='active';} else { $class='next1';} ?>" href='".SITEPATH.slugify($brandName)."-gadgets?page=<?=$total_pages."&".$params1;?>'>Last&ensp;<span class="fa fa-angle-right"></span></a></li>
                            </ul>
            </div> </div>
