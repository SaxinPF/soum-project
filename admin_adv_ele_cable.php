		  ...
		  //echo '<pre>';
		    ...
		    if(isset($_POST['prod_id'])){
                ...
                $model = $drpModel=mysqli_real_escape_string($db,$_POST['modal_name']);
						...
						if($_POST['act' ] == "add"){
							...
							$table=(!isset($poster_type)?'soum_product_master_temp':'soum_product_master');
								if($oldphone==2){
								 ...
								 $s2=1;
								 $s3='cable';
									$sql=$db->prepare("insert into $table (post_date,titile,prod_cat_id,brand,modal_name,imei_no,images,images1,images2,description,poster_type,poster_id,active,appliable_rate,offer_price,warranty,color_name,seller_name,seller_email,seller_phone,seller_address,seller_city,opening_stock,current_stock,category_type)
									values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
									$sql->bind_param("sssssssssssssssssssssssss",$post_date,$titile,$oldphone,$drpBrand,$drpModel,$imi_no,$img1,$img2,$img3,$description,$poster_type,$poster_id,$approve,$offer_price,$offer_price,$warranty,$color_name,$seller_name,$seller_email,$seller_phone,$seller_address,$seller_city,$s1,$s2,$s3);
							...
							<form id="uploadimage"  method="post" enctype="multipart/form-data">
									...
									if($approve==1){
									  $check = 'checked="checked"';
											...
											<label><input name="approve" <?php echo $check; ?>type="checkbox" id="approve"  /></label>
									</div>
									<div class="col-md-12">
...
<script>
		...
		function savefun(){
			...
			$.ajax({
				type:"Post",
				url: "admin_adv_finish.php",
				data:{"brand":p,'approve':approve,'token':token},
				dataType: 'json',
				success:function(res){
					if(res.status==1){
					   window.location.href="admin_adv_cable.php?admin_adv_s=Ad has been updated successfully";
					}
				}
			});
