			          ...
			          if($prodid=='')	{
							...
							/* 	foreach($condition as $a=>$b)
								...
								if($oldphone==2)
								{
								 ...
								 $s2=1;
								 $exchange=1;
									$sql=$db->prepare("insert into $table (post_date,titile,prod_cat_id,brand,modal,imei_no,images,images1,images2,Constituent,poster_type,poster_id,active,appliable_rate,offer_price,rom,ram2,exchange)
									values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
									$sql->bind_param("ssssssssssssssssss",$post_date,$titile,$oldphone,$drpBrand,$drpModel,$imi_no,$img1,$img2,$img3,$txt_description,$poster_type,$poster_id,$active,$offer_price,$offer_price,$rom,$ram,$exchange);
										...
										if($res)
									{
									   ...
									   $sqlm="select * from soum_prod_subsubcat where prod_subsubcat_id=$drpModel";
										...
										?>
										  ...
										  <?php    $order_prod_id =   $_REQUEST['order_prod_id']; ?>
										  <?php    $prodname1 =     $_REQUEST['order_prod_name']; ?>
												   <div class="errorMessage233" style="color:red;"></div>
													   <input type="hidden" name="mobile" value="<?=$mobile_no?>" id="mobile2" class="form-control"  placeholder="">
													   <input name="prod_id" value="<?=$order_prod_id?>" type="hidden" />
													   <input type="hidden" name="name" id="name" value="<?=$name?>" class="form-control"  aria-describedby="emailHelp" placeholder="">
														<input   type="hidden" class="form-control" name="desc" value="Hello SOUM Team, I wish to buy "<?=$prodname1;?>", and would like to discuss about its price. I have shared my phone number so you could call me anytime."/>
										  ...
										  function Orderform(){
												$("#btorder").html("Please Wait .......<img src='upload/loader.gif' style='width:50px!important;height:50px!important;'>");
												$('.errorMessage233').html('&nbsp;');
											var frmData = $("#OrderForm").serialize();
											 $.ajax({
													type: "POST",
													url: 'order_detail_exchange.php',
													data: frmData,
													dataType: 'json',
										  ...
										  <p style="text-align:center" id="btorder" > </p>
										<?php
										 
										 
									    echo "<p id='intrested' style='text-align:center;font-size:16px;font-weight:500;margin-top:20px;display:none;'>$msg</br></br><a href='customer_dashboard.php' class='theme-btn btn-style-four' style='text-decoration:none;font-size:20px;padding:5px 15px;'>Go to Home</a>";
						...
						{  //Edit Case nahi hai only code is here.
						        if($oldphone==2){
							      $qty=1;
							    }else{
							        $qty=$qty;
 ...
 */
							...
							$sql=$db->prepare("update soum_product_master set titile='$titile',brand='$drpBrand',modal='$drpModel',imei_no='$imi_no',rom='$rom',ram2='$ram',appliable_rate='$offer_price',active=0,offer_price='$offer_price' where prod_id=$prodid");
						    $res=$sql->execute();
						    $enq_id=$prodid;
						/***************Image1************************************/
									if($imageData1!='')
